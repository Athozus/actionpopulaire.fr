...
def send_mail_new_member_message_to_referents(message_pk):
    ...
    subject = clean_subject_email(
        f"Accueillir votre nouveau membre par message, {message.participant.display_name} !"
    )
...
def send_message_new_member(message_pk):
    message = SupportGroupMessage.objects.get(pk=message_pk)
    send_message_to(message, [message.participant])
...
def send_newly_certified_group_notifications(supportgroup_pk):
    ...
    send_template_email(
        ...
        recipients=[*recipients, settings.EMAIL_SUPPORT, settings.EMAIL_GA],
    )
