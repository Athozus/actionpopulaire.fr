...
const MAPPING_COMMON_PARAMS_TO_CRISPY = Object.keys(
  MAPPING_COMMON_PARAMS_TO_FORM_BUILDER,
).reduce((acc, current) => {
  return {
    ...acc,
...
function mapFormBuilderParametersToCrispy(formField, crispyField) {
  return mapParameterToField(
    formField,
    crispyField,
    MAPPING_COMMON_PARAMS_TO_CRISPY,
  );
}
...
const MAPPING_TYPE_TO_CRISPY = Object.keys(MAPPING_TYPE_TO_FORM_BUILDER).reduce(
  (acc, current) => {
    return {
      ...acc,
